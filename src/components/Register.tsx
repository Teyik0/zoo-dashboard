...
const Register = () => {
  ...
  const handleClick = () => {
    if (isLogin) {
      ...
      if (profileForm.lastName.length < 3) {
      ...
      fetch(`${'http://localhost:3000'}/auth/register`, {
        ...
        if (res.status === 200) {
          res
            ...
            .then((session) => {
              setSession(session);
              toast.success('Account created', { id: notif });
            })
            .catch((err) => {
              console.log(err);
              toast.error('An error has occured', { id: notif });
            });
          console.log(session);
        } else {
          toast.error('An error has occured', { id: notif });
        ...
        <section className='relative flex h-32 items-end bg-gray-900 lg:col-span-5 lg:h-full xl:col-span-6'>
          ...
          <div className='hidden lg:relative lg:block lg:p-12'>
            ...
            <p className='mt-4 leading-relaxed text-white/90'>
              Contrôle l&apos;accès à ton espace de travail.
            </p>
          </div>
        </section>
            ...
            <form action='#' className='mt-8 grid grid-cols-6 gap-6'>
              ...
              <div
                className={`col-span-6 ${
                  isLogin ? 'sm:col-span-6' : 'sm:col-span-3'
                }`}
              >
                <label
                  htmlFor='Password'
